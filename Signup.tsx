    ...
    <div className="min-h-screen bg-black text-white pt-24 pb-20">
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <motion.div 
          ...
          className="mb-20"
        >
          <div className="bg-ngt-dark-gray/30 rounded-3xl border border-white/5 p-8 md:p-12">
            <h2 className="text-2xl md:text-3xl font-serif italic gold-text mb-12 text-center">Performance & Fiabilité — Indicateurs Clés</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                { 
                  ...
                  value: "~1ms", 
                  sub: "vs 2–5s pour un humain", 
                  icon: <Zap size={24} className="text-ngt-gold" /> 
                },
                { 
                  label: "Disponibilité", 
                  value: "24/7", 
                  sub: "Aucune interruption", 
                  icon: <Activity size={24} className="text-ngt-gold" /> 
                },
                { 
                  label: "Biais émotionnel", 
                  value: "0%", 
                  sub: "Discipline absolue", 
                  icon: <Target size={24} className="text-ngt-gold" /> 
                },
                { 
                  label: "Taux de réussite moyen", 
                  value: "40–65%", 
                  sub: "Selon stratégie & marché", 
                  icon: <TrendingUp size={24} className="text-ngt-gold" /> 
                },
              ].map((item, idx) => (
                <div key={idx} className="flex flex-col items-center text-center p-6 rounded-2xl bg-white/[0.02] border border-white/5 hover:border-ngt-gold/30 transition-all hover:bg-ngt-gold/5 group">
                  <div className="mb-4 p-3 rounded-xl bg-ngt-gold/10 group-hover:scale-110 transition-transform">{item.icon}</div>
                  <p className="text-[10px] uppercase tracking-widest text-gray-500 mb-2">{item.label}</p>
                  <p className="text-3xl font-serif italic text-white mb-1">{item.value}</p>
                  <p className="text-[10px] text-ngt-gold font-medium">{item.sub}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mb-20">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-serif italic gold-text mb-4">Profils Utilisateurs</h2>
            <p className="text-gray-400 max-w-2xl mx-auto">L'algorithme Caméléon s'adapte à vos objectifs, que vous soyez un professionnel ou un passionné.</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <motion.div 
              ...
              viewport={{ once: true }}
              className="group p-8 rounded-3xl bg-ngt-dark-gray/30 border border-white/5 hover:border-ngt-gold/30 transition-all relative overflow-hidden"
            >
              <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:opacity-20 transition-opacity">
                <Briefcase size={80} className="text-ngt-gold" />
              </div>
              <div className="mb-8 flex items-center gap-4">
                <div className="p-4 rounded-2xl bg-ngt-gold/10 text-ngt-gold"><Briefcase size={24} /></div>
                <div>
                  <h3 className="text-xl font-serif italic gold-text">Salarié</h3>
                  <p className="text-[10px] uppercase tracking-widest text-gray-500">Revenu complémentaire passif</p>
                </div>
              </div>
              <div className="space-y-6">
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-ngt-gold mb-2 font-bold italic">Passif • Automatisé • Flexible</p>
                  <ul className="space-y-2">
                    {[
                      "Aucune présence requise — l'EA tourne pendant les heures de bureau.", 
                      "Diversification — revenu parallèle sans quitter l'emploi.", 
                      "Discipline imposée — pas d'impulsivité après une journée stressante."
                    ].map((p, i) => (
                      <li key={i} className="flex items-start gap-2 text-xs text-gray-400">
                        <Check size={14} className="text-ngt-gold shrink-0 mt-0.5" /> {p}
                      </li>
                    ))}
                  </ul>
                </div>
                <div className="pt-4 border-t border-white/5">
                  <p className="text-[10px] uppercase tracking-widest text-gray-500 mb-1">Capital requis</p>
                  <p className="text-sm font-medium">500€ min. pour un EA sérieux</p>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:opacity-20 transition-opacity">
                <GraduationCap size={80} className="text-ngt-gold" />
              </div>
              <div className="mb-8 flex items-center gap-4">
                <div className="p-4 rounded-2xl bg-ngt-gold/10 text-ngt-gold"><GraduationCap size={24} /></div>
                <div>
                  <h3 className="text-xl font-serif italic gold-text">Étudiant</h3>
                  <p className="text-[10px] uppercase tracking-widest text-gray-500">Formation & revenus</p>
                </div>
              </div>
              <div className="space-y-6">
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-ngt-gold mb-2 font-bold italic">Apprentissage • Backtesting • Faible capital</p>
                  <ul className="space-y-2">
                    {[
                      "Backtesting illimité — tester des stratégies sans risque réel.", 
                      "Comprendre les marchés — lire le code = lire la logique des prix.", 
                      "Comptes démo — démarrer à 0€ avec des plateformes comme MT4/MT5."
                    ].map((p, i) => (
                      <li key={i} className="flex items-start gap-2 text-xs text-gray-400">
                        <Check size={14} className="text-ngt-gold shrink-0 mt-0.5" /> {p}
                      </li>
                ...
                <div className="pt-4 border-t border-white/5">
                  <p className="text-[10px] uppercase tracking-widest text-gray-500 mb-1">Risque principal</p>
                  <p className="text-sm font-medium">Over-optimisation (\"curve fitting\") sur les données passées</p>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:opacity-20 transition-opacity">
                <Layers size={80} className="text-ngt-gold" />
              </div>
              <div className="mb-8 flex items-center gap-4">
                <div className="p-4 rounded-2xl bg-ngt-gold/10 text-ngt-gold"><BarChart3 size={24} /></div>
                <div>
                  <h3 className="text-xl font-serif italic gold-text">Trader actif</h3>
                  <p className="text-[10px] uppercase tracking-widest text-gray-500">Scalabilité & rigueur</p>
                </div>
              </div>
              <div className="space-y-6">
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-ngt-gold mb-2 font-bold italic">Multi-marchés • Scalable • Technique</p>
                  <ul className="space-y-2">
                    {[
                      "Multi-paires simultanées — un EA sur 20 paires en même temps.", 
                      "Backtesting robuste — optimisation sur données historiques longues.", 
                      "Élimination du biais cognitif — fomo, revenge trading éliminés."
                    ].map((p, i) => (
                      <li key={i} className="flex items-start gap-2 text-xs text-gray-400">
                        <Check size={14} className="text-ngt-gold shrink-0 mt-0.5" /> {p}
                      </li>
                ...
                <div className="pt-4 border-t border-white/5">
                  <p className="text-[10px] uppercase tracking-widest text-gray-500 mb-1">Surveillance nécessaire</p>
                  <p className="text-sm font-medium">Les marchés évoluent, l'EA doit être mis à jour</p>
                </div>
              </div>
            </motion.div>
          </div>
        ...
        <motion.div 
          ...
          className="mb-20"
        >
          <div className="bg-ngt-dark-gray/30 rounded-3xl border border-white/5 overflow-hidden shadow-2xl">
            <div className="p-8 md:p-10 border-b border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                <h2 className="text-2xl font-serif italic gold-text mb-1 flex items-center gap-3">
                  <Activity className="text-ngt-gold" size={24} />
                  Paramètres marché — analyse & recommandations
                </h2>
                <p className="text-xs text-gray-500 uppercase tracking-widest">Guide technique pour une optimisation maximale</p>
              </div>
              <div className="px-4 py-2 bg-ngt-gold/10 border border-ngt-gold/20 rounded-full flex items-center gap-2">
                <CheckCircle2 size={14} className="text-ngt-gold" />
                <span className="text-[10px] uppercase tracking-widest font-bold text-ngt-gold">Recommandé par NGT</span>
              </div>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-white/5">
                    <th className="px-8 py-5 text-[10px] uppercase tracking-widest text-gray-500 font-bold">Marché</th>
                    <th className="px-8 py-5 text-[10px] uppercase tracking-widest text-gray-500 font-bold">maxSpread actuel</th>
                    <th className="px-8 py-5 text-[10px] uppercase tracking-widest text-gray-500 font-bold">Recommandé</th>
                    <th className="px-8 py-5 text-[10px] uppercase tracking-widest text-gray-500 font-bold">minRangeH1</th>
                    <th className="px-8 py-5 text-[10px] uppercase tracking-widest text-gray-500 font-bold">Commentaire</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5 font-mono text-[11px]">
                  {[
                    { market: "GOLD/XAU", actual: "80", rec: "80 ✓", range: "200×pt", comment: "OK pour horaires normaux" },
                    { market: "EURUSD", actual: "30", rec: "25", range: "50×pt", comment: "Réduire légèrement" },
                    { market: "GBPUSD", actual: "30", rec: "35", range: "50×pt", comment: "Tolérance légèrement basse" },
                    { market: "USDJPY", actual: "30", rec: "30 ✓", range: "50×pt", comment: "OK" },
                    { market: "AUDUSD", actual: "30", rec: "25", range: "50×pt", comment: "Souvent sous 20pts" },
                    { market: "OIL", actual: "50", rec: "80", range: "60×pt → 150×pt", comment: "Spread WTI souvent 60-100pts" },
                    { market: "BTC", actual: "200", rec: "500", range: "1000×pt → 5000×pt", comment: "Spread crypto variable 200-800pts" },
                    { market: "DAX/GER40", actual: "40", rec: "20", range: "80×pt ✓", comment: "DAX spread typique 10-15pts" },
                  ].map((item, idx) => (
                    <tr key={idx} className="hover:bg-white/5 transition-colors group">
                      <td className="px-8 py-4 font-bold text-white group-hover:text-ngt-gold transition-colors font-sans text-sm italic">{item.market}</td>
                      <td className="px-8 py-4 text-gray-400">{item.actual}</td>
                      <td className={`px-8 py-4 ${item.rec.includes('✓') ? 'text-green-500' : 'text-ngt-gold font-bold'}`}>{item.rec}</td>
                      <td className="px-8 py-4 text-gray-400">{item.range}</td>
                      <td className="px-8 py-4 text-gray-500 italic font-sans">{item.comment}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
